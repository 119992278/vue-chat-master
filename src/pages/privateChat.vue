<template>
  <!--  私聊 -->
  <div class="wrapper">
    <Header goback='true' userInfo='true' :chatTitle="someOneInfoGetter.remark ? someOneInfoGetter.remark : someOneInfoGetter.name"></Header>
    <ul>
...
export default {
  ...
  methods: {
    ...
    isFriend () {
      axios
        ...
        .then(res => {
          this.isMyFriend = res.data.data.isMyFriend.length !== 0
          this.isHisFriend = res.data.data.isHisFriend.length !== 0
        })
        .catch(err => {
          const errorMsg = err.response.data.error
          this.$message({
  ...
  created () {
    ...
    this.getPrivateMsg()
    this.getMsgBySocket()
    console.log('$store.dispatch写入数据to_user=' + this.toUserInfo.to_user)
    this.$store.dispatch('someOneInfoAction', this.toUserInfo.to_user)
    this.inputMsg = '我是' + this.fromUserInfo.name + ',你好'
  }
}
</script>
